...
public:
	...
	void save_diversity(char saveFilename[1024]);
...
long double MOEA::Scalarization_ASF(std::vector<double> y_obj, double w){
...
void MOEA::update_diversity_factor()
{
	...
	//cout<<Initial_lowest_distance_factor<<" "<<ratio<<" "<<lowestDistanceFactor<<endl;
}
...
// Copia el vector de fuerzas del caso de carga, en caso de que se vaya a optimizar
	...
	sprintf(filename1,"INDIVIDUOS_%s_MOEALS_seed_%d_nvar_%d_ngen_%lld", strTestInstance, run, nvar, max_ngen);
	sprintf(filename2,"FRENTE_%s_MOEALS_seed_%d_nvar_%d_ngen_%lld", strTestInstance, run, nvar, max_ngen);
	sprintf(filename3,"DIVERSITY_%s_MOEALS_seed_%d_nvar_%d_ngen_%lld", strTestInstance, run, nvar, max_ngen);
	

    save_front(filename2); //save the objective space information
	save_pos(filename1); //save the decision variable space information
	save_diversity(filename3); //save the decision variable space information
	...
	while(generation <= max_ngen)
	{
		...
		Assign_weights(child_pop,w_v);
 		...
 		//cout<<diversity<<" "<<lowestDistanceFactor<<endl;

	    if(generation%500==0 )
	    {	
          save_front(filename2); //save the objective space information
	      save_pos(filename1); //save the decision variable space information
	      save_diversity(filename3);
...
void MOEA::save_diversity(char saveFilename[1024])
{
	std::fstream fout;
	//fout.open(saveFilename,std::ios::out);
	fout.open(saveFilename,fstream::app|fstream::out );
	//cout<<saveFilename<<endl;
	fout<<generation<<" "<<diversity<<" "<<lowestDistanceFactor;
	fout<<"\n";
	fout.close();
