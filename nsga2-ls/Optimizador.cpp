...
///////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    sprintf(buf1, "FRENTES_%d_%d_nsga_ls_5000",nelem, prueba);
    sprintf(buf2, "INDIVIDUO_%d_%d_nsga_ls_5000",nelem, prueba);
...
double beta, alfa, penaliza, dd, de, deltaf, fact,D;
...
// Parametros para el recosido simulado
   ...
   // Num_iter = 1000;
    ...
    for (int i = 0; i < poblacion.size(); ++i)
    {
                ...
                fflush(p1File);
                fprintf(p1File,"%lf ",poblacion[i].y_obj[0]);
                fflush(p1File);
                fprintf(p1File,"%lf ",poblacion[i].y_obj[1]);
                fflush(p1File);
    ...
    for (int gen = 2; gen<= 5000; gen++)
    {
    	...
    	Weight_vectors(hijos);

		Assign_weights(hijos,w_v,ishot, npnod,nelem, npres,  ncarg, ntipo, ngdln, nmats,
                 ndime, iwrit, isale, nreso, nincl, nfami, ncaso,
                  nnode, mnode, nprop, ngaus, ntens, nevab, indso, ntotv,
                 lnods, inpre, nareas, narerf, narerc, narecon,
                 lreso, lincl, ntips, nodpr, matnu, matva, matvm,
...
void Weight_vectors(std::vector<Individuo> child){//this could  be randomly.. 

	double delta=1.0/(double)(child.size()-1);
	w_v.clear();
...
{ 
    ...
    {
    	population[i].y_obj.clear();
    	population[i].y_obj.resize(2);

    	population[i].y_obj[0]= (population[i].objs[0]-(0.8*ideal[0]))/(nadir[0] - (0.8*ideal[0]));
		
		population[i].y_obj[1]=(population[i].objs[1]-(0.8*ideal[1]))/(nadir[1] - (0.8*ideal[1]));
    }
...
void Assign_weights(std::vector<Individuo> &child, std::vector<double> w,int ishot, int npnod, int nelem, int npres, int ncarg, int ntipo, int ngdln, int nmats,
	...
	std::vector<Individuo> child2=child;
	
	objtosort=0;//sort by first objective

	sort(child2.begin(),child2.end(),sortbyobjs);

	for (int k = 0; k < child2.size(); ++k)
	{
		

		       	Local_search(child2[k],w[k],ishot, npnod,nelem, npres,  ncarg, ntipo, ngdln, nmats,
                 ndime, iwrit, isale, nreso, nincl, nfami, ncaso,
                  nnode, mnode, nprop, ngaus, ntens, nevab, indso, ntotv,
                 lnods, inpre, nareas, narerf, narerc, narecon,
                 lreso, lincl, ntips, nodpr, matnu, matva, matvm,
...
long double Scalarization_ASF(std::vector<double> y_obj, double w){
	...
	double fit,epsilon=1e-10;

	//cout<<"AQUI 1 "<<y_obj[0]/(w+epsilon)<<" "<<y_obj[1]/(1.0-w)<<endl;
	//cout<<"AQUI 2 "<<y_obj[0]<<" "<<w<<" "<<y_obj[1]<<" "<<(1.0-w)<<endl;
	fit=max(y_obj[0]/(w+epsilon),y_obj[1]/(1.0-w+epsilon));
	//cout<<"fit "<<fit<<endl;
	//exit(0);
...
void Local_search(Individuo &ind, double w,int ishot, int npnod, int nelem, int npres, int ncarg, int ntipo, int ngdln, int nmats,
	...
	fitness=Scalarization_ASF(ind.y_obj,w);
	std::vector<double> v;
	...
	for (int k = 0; k < 75; ++k)
	{
		for (int i = 0; i < ind.secciones.size(); ++i)
		{
			matva[i+1]=ind.secciones[i];
		}
		modif=aleatorioEntero(1,ind.secciones.size());//rand()%(poblacion[i].secciones.size());//cual posicion voy a modificar

		do{
            secc=aleatorioEntero(1,59);
        }while(secc==ind.secciones[modif-1]);

        matva[modif]=secc;

       	evaluaciones++;
        v_original=Funcion_fitness(ishot, npnod,nelem,npres,ncarg,ntipo,ngdln,nmats,ndime,iwrit,isale,nreso,nincl,
        ...
        fitness_ls=Scalarization_ASF(v,w);

        if (fitness_ls<fitness)
        {
        	fitness=fitness_ls;
        	ind.secciones[modif-1]=secc;
        	ind.y_obj=v;
        	ind.peso=valor[4];
        	ind.desplazamiento=valor[2];;
        	ind.objs[0]=v_original[0];
        	ind.objs[1]=v_original[1];
            ind.efi=valor[3];
            ind.sum_pen=v_original[2]+v_original[3];;
        }
...
//
    ...
    //for (int i = 0; i < 1; ++i)
    {
       ...
       v=Funcion_fitness(ishot, npnod,nelem,npres,ncarg,ntipo,ngdln,nmats,ndime,iwrit,isale,nreso,nincl,//QUEDE AQUIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII
                                                  ...
                                                  vectp,vectu,listaCatalogos,ubiCat,vectores_fuerzas,npesp,efi_max,stiff,
                                                  neqns, nwktl);//@mg'
         poblacion[i].y_obj.resize(2);

        poblacion[i].y_obj[0]= ( v[0]-(0.8*ideal[0]))/(nadir[0] - (0.8*ideal[0]));//normalizing 

        poblacion[i].y_obj[1]= ( v[1]-(0.8*ideal[1]))/(nadir[1] - (0.8*ideal[1]));//normalizing 
