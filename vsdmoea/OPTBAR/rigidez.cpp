 ...
 ******************************************************************************/
{
	...
	for(ielem=1; ielem<=nelem; ielem++) {
		...
		xarea = props[lprop][2];
		xiner = props[lprop][3];
		//printf("lprop %d young %lf xarea %lf xiner %lf\n", lprop,young,xarea,xiner);
		//if(ndime ==3)
        {
			yiner = props[lprop][4];
			xjner = props[lprop][5];
		...
		//printf("gcort %lf xjner %lf yiner %lf\n", gcort,xjner,yiner);
	...
	//printf("fiiiiiiiiiiiin SRMAAAAAAAAT \t RIGIIIIIIID  \n");
